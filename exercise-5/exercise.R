...
most_popular_in_year <- function(year){
  ...
  return(names_year$name[names_year$prop == max(names_year$prop)])
}
...
number_in_million <- function(name, year){
  names_year <- names[names$year == year, ]
  name_prop <- names_year$prop[names_year$name == name]
  return(round(name_prop * 1000000))
}
...
alex_1997 <- number_in_million('Alex', 1997)
